...
const leaveSchema=z.object({
  ...
  motivo:z.string().trim().min(5).max(2000),cidSid:z.string().trim().max(20).optional().default(''),documento:z.object({nome:z.string().min(1).max(255),dados:z.string().min(20).max(7500000)}).nullable().optional(),
...
function requireMedicalDocument(data,document,existingDocument=false){
  if(data.tipo==='licenca_medica'&&!data.cidSid?.trim())throw httpError(400,'Informe o CID/SID que consta no atestado médico.');
...
router.get('/',async(request,response,next)=>{try{
  const {rows}=await pool.query(`SELECT a.id,a.usuario_id AS "usuarioId",u.nome,u.matricula_funcional AS matricula,a.tipo,a.data_inicio AS "dataInicio",a.data_fim AS "dataFim",a.retorno_previsto AS "retornoPrevisto",a.motivo,a.cid_sid AS "cidSid",a.alerta_secretaria_emitido_em AS "alertaSecretariaEmitidoEm",a.status,a.documento_nome AS "documentoNome",(a.data_fim-a.data_inicio+1) AS "diasCorridos",CASE WHEN a.tipo='licenca_medica' AND (a.data_fim-a.data_inicio+1)>30 THEN TRUE ELSE FALSE END AS "alertaAtestadoLongo",COALESCE(es.escolas,'Rede municipal') AS escola,
...
router.post('/',async(request,response,next)=>{const client=await pool.connect();try{
  ...
  const longMedical=data.tipo==='licenca_medica'&&calendarDays(data.dataInicio,data.dataFim)>30;
  const {rows}=await client.query(`INSERT INTO afastamentos_funcionais(usuario_id,tipo,data_inicio,data_fim,retorno_previsto,motivo,cid_sid,alerta_secretaria_emitido_em,documento_nome,documento_mime,documento_tamanho,documento_bytes,criado_por,atualizado_por) VALUES($1,$2,$3,$4,$5,$6,NULLIF($7,''),CASE WHEN $8 THEN NOW() ELSE NULL END,$9,$10,$11,$12,$13,$13) RETURNING id,status,alerta_secretaria_emitido_em AS "alertaSecretariaEmitidoEm"`,[data.usuarioId,data.tipo,data.dataInicio,data.dataFim,data.retornoPrevisto,data.motivo,data.tipo==='licenca_medica'?data.cidSid:'',longMedical,document?.nome||null,document?.mime||null,document?.bytes.length||null,document?.bytes||null,request.user.sub]);
  ...
  await client.query('COMMIT');return response.status(201).json({id:rows[0].id,diasCorridos:calendarDays(data.dataInicio,data.dataFim),alertaAtestadoLongo:longMedical,alertaSecretariaEmitidoEm:rows[0].alertaSecretariaEmitidoEm,message:longMedical?'Atestado registrado e alerta emitido para a Secretaria de Educação.':'Férias ou afastamento registrado com sucesso.'});
...
router.patch('/:id',async(request,response,next)=>{const client=await pool.connect();try{
  ...
  const longMedical=data.tipo==='licenca_medica'&&calendarDays(data.dataInicio,data.dataFim)>30;
  await client.query(`UPDATE afastamentos_funcionais SET tipo=$1,data_inicio=$2,data_fim=$3,retorno_previsto=$4,motivo=$5,cid_sid=CASE WHEN $1='licenca_medica' THEN NULLIF($6,'') ELSE NULL END,alerta_secretaria_emitido_em=CASE WHEN $7 THEN COALESCE(alerta_secretaria_emitido_em,NOW()) ELSE NULL END,documento_nome=COALESCE($8,documento_nome),documento_mime=COALESCE($9,documento_mime),documento_tamanho=COALESCE($10,documento_tamanho),documento_bytes=COALESCE($11,documento_bytes),atualizado_por=$12,atualizado_em=NOW() WHERE id=$13`,[data.tipo,data.dataInicio,data.dataFim,data.retornoPrevisto,data.motivo,data.cidSid,longMedical,document?.nome||null,document?.mime||null,document?.bytes.length||null,document?.bytes||null,request.user.sub,request.params.id]);
  ...
  await client.query('COMMIT');const diasCorridos=calendarDays(data.dataInicio,data.dataFim);return response.json({id:Number(request.params.id),diasCorridos,alertaAtestadoLongo:longMedical,message:longMedical?'Período atualizado e alerta emitido para a Secretaria de Educação.':'Período atualizado com sucesso.'});
}catch(error){await client.query('ROLLBACK');return next(error);}finally{client.release();}});
