...
router.get('/',async(request,response,next)=>{try{
  const {rows}=await pool.query(`SELECT a.id,a.usuario_id AS "usuarioId",u.nome,u.matricula_funcional AS matricula,a.tipo,a.data_inicio AS "dataInicio",a.data_fim AS "dataFim",a.retorno_previsto AS "retornoPrevisto",a.motivo,a.status,a.documento_nome AS "documentoNome",COALESCE(es.escolas,'Rede municipal') AS escola,
  ...
  return response.json({dados:rows,resumo:{emFerias:active.filter(i=>i.tipo==='ferias').length,afastados:active.filter(i=>i.tipo!=='ferias').length,retornosProximos:upcoming.length,pendencias:pending.length},alertas:upcoming.slice(0,8)});
}catch(error){return next(error);}});

router.post('/',async(request,response,next)=>{const client=await pool.connect();try{
  const data=createSchema.parse(request.body);const document=decodeDocument(data.documento);await client.query('BEGIN');const user=await targetUser(client,data.usuarioId,request);
  const overlap=await client.query(`SELECT 1 FROM afastamentos_funcionais WHERE usuario_id=$1 AND status='confirmado' AND DATERANGE(data_inicio,data_fim,'[]')&&DATERANGE($2::date,$3::date,'[]') LIMIT 1`,[data.usuarioId,data.dataInicio,data.dataFim]);
  ...
  await client.query('COMMIT');return response.status(201).json({id:rows[0].id,message:'Férias ou afastamento registrado com sucesso.'});
