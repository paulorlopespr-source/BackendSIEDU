DO $$
DECLARE v_usuario BIGINT; v_professor BIGINT; v_escola INTEGER; v_turma BIGINT; v_aluno BIGINT; nomes TEXT[]:=ARRAY['Ana Clara Souza','Bruno Henrique Lima','Mariana Alves Santos']; aluno_nome TEXT; idx INTEGER:=0;
BEGIN
 ...
 FOREACH aluno_nome IN ARRAY nomes LOOP
  idx:=idx+1;
  SELECT id INTO v_aluno FROM alunos WHERE nome_completo=aluno_nome LIMIT 1;
  IF v_aluno IS NULL THEN INSERT INTO alunos(nome_completo,data_nascimento,ativo,criado_por) VALUES(aluno_nome,DATE '2013-03-10'+idx,TRUE,v_usuario) RETURNING id INTO v_aluno; END IF;
